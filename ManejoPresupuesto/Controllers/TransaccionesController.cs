...
namespace ManejoPresupuesto.Controllers
{
	public class TransaccionesController: Controller
	{
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            var transaction = await repositorioTransacciones.ObtenerPorId(id, usuarioId);
            if(transaction is null)
            {
                return RedirectToAction("NoEncontrado", "Home");
            }

            var modelo = mapper.Map<TransaccionActulizacionViewModel>(transaction);
            ...
            modelo.CuentaAnteriorId = transaction.CuentaId;
            modelo.Categorias = await ObtenerCategorias(usuarioId, transaction.TipoOperacionId);
            modelo.Cuentas = await ObtenerCuentas(usuarioId);
            return View(modelo);
        ...
        public async Task<IActionResult> Editar(TransaccionActulizacionViewModel modelo)
        {
            ...
            if (cuenta is null)
            {
                logger.LogInformation("cuenta is null");
                return RedirectToAction("NoEncontrado", "Home");
            }
            ...
            if (categoria is null)
            {
                logger.LogInformation("categoria is null");
                return RedirectToAction("NoEncontrado", "Home");
            }
